...
def inverse_normal_cdf(p: float, mu: float = 0, sigma: float = 1, tolerance: float = 0.00001) -> float:
    ...
    low_z = -10.0  # normal_cdf(-10.0) is (very close to) 0
    hi_z = 10.0    # normal_cdf(10.0) is (very close to) 1
...
def binomial_histogram(p: float, n: int, num_points: int) -> None:
    ...
    plt.bar([x - 0.4 for x in histogram.keys()],
            scalar_divide(num_points, histogram.values()),
            0.8,
            color="0.75")
    ...
    ys = [normal_cdf(x + 0.5, mu, sigma) - normal_cdf(x - 0.5, mu, sigma)
          for x in xs]
    plt.plot(xs, ys)
...
def main():
    ...
    # to make out code more descriptive and readable.
    class Kid(enum.Enum):
        BOY = 0
        GIRL = 1
